...
用户操作（开始/停止/补录）
  ...
  ① 写入思源数据库          ← 毫秒级，永不失败
       │
       ▼
  ② 尝试推送到 Toggl API    ← 可能失败（配额/网络）
       │
       ├─ 成功 → 标记「正常」
       │
       └─ 失败 → 标记「本地待上传」或「Toggl 待更新」→ 下次同步自动重试
```

**好处**：API 配额耗尽或网络中断时，操作不受影响。所有次数都会在 API 恢复后自动上传。
**数据库字段**：关闭时填入「结束」和「时长」，开始计时时「结束」为空、时长显示「进行中」。

...
export default class TogglSyncPlugin extends Plugin {
    ...
    openSetting() {
        ...
        const dialog = new Dialog({
            ...
            content: `<div class="toggl-sync__settings">
                <div class="toggl-sync__card">
                    <div class="toggl-sync__card-header">连接</div>
                    <div class="toggl-sync__card-body">
                        <div class="toggl-sync__field">
                            <div class="toggl-sync__field-label">${this.i18n.token}</div>
                            <div class="toggl-sync__input-row">
                                <input id="ts-token" class="b3-text-field" type="password" placeholder="Toggl API Token" value="${
                this.escapeHtml(this.config.token)
            }">
                                <button id="ts-verify" class="b3-button b3-button--outline" type="button">${this.i18n.tokenVerify}</button>
                            </div>
                            <div class="toggl-sync__field-hint">${this.i18n.tokenDesc}</div>
                        </div>
                        <div class="toggl-sync__field">
                            <label class="toggl-sync__switch">
                                <input id="ts-api-enabled" type="checkbox" class="b3-switch" ${
                this.config.apiEnabled !== false ? "checked" : ""
            }>
                                <span>启用 Toggl API</span>
                            </label>
                            <div class="toggl-sync__field-hint">关闭后所有操作仅在本地运行，不上传也不下拉 Toggl</div>
                        </div>
                    </div>
                </div>

                <div class="toggl-sync__card">
                    <div class="toggl-sync__card-header">同步</div>
                    <div class="toggl-sync__card-body">
                        <div class="toggl-sync__field">
                            <div class="toggl-sync__field-label">${this.i18n.targetDoc}</div>
                            <input id="ts-doc" class="b3-text-field toggl-sync__input-wide" placeholder="Document ID" value="${
                this.escapeHtml(this.config.targetDocId)
            }">
                            <div class="toggl-sync__field-hint">${this.i18n.targetDocDesc}</div>
                        </div>
                        <div class="toggl-sync__field-row">
                            <div class="toggl-sync__field toggl-sync__field--half">
                                <div class="toggl-sync__field-label">首次 / 修复范围</div>
                                <select id="ts-days" class="b3-select">
                                    <option value="7" ${
                ...
                this.config.initialDays === 90 ? "selected" : ""
                            ...
                            <div class="toggl-sync__field toggl-sync__field--half">
                                <div class="toggl-sync__field-label">自动同步周期</div>
                                <select id="ts-auto-sync" class="b3-select">
                ...
                <div class="toggl-sync__card">
                    <div class="toggl-sync__card-header">状态栏</div>
                    <div class="toggl-sync__card-body">
                        <div class="toggl-sync__field">
                            <label class="toggl-sync__switch">
                ...
                this.config.statusBarTimer ? "checked" : ""
                        ...
                        <div class="toggl-sync__field">
                            <div class="toggl-sync__field-label">${this.i18n.statusText}</div>
                            <input id="ts-statusbar-text" class="b3-text-field" placeholder="Toggl" value="${
                ...
                <div class="toggl-sync__card">
                    <div class="toggl-sync__card-header">维护</div>
                    <div class="toggl-sync__card-body">
                        <div class="toggl-sync__meta">
                            <span class="toggl-sync__meta-key">${this.i18n.lastSync}</span>
                            <span class="toggl-sync__meta-value" id="ts-lastSync">${lastSync}</span>
                            <button id="ts-clearSync" class="b3-button b3-button--text toggl-sync__meta-action" type="button">清空</button>
                        </div>
                        <div class="toggl-sync__btn-row">
                        ...
                        <div class="toggl-sync__field-hint" style="margin-top:6px;">
                            <span class="toggl-sync__text-warn">清理操作执行前会弹出二次确认</span>
                        </div>
                    </div>
                </div>

                <div class="toggl-sync__card toggl-sync__card--aux">
                    <div class="toggl-sync__card-header">诊断</div>
                    <div class="toggl-sync__card-body">
                        <div class="toggl-sync__field-hint" style="margin-top:-4px;margin-bottom:10px;">排查工具 —— 仅遇到异常时使用</div>
                        <div class="toggl-sync__btn-row">
                        ...
                        <div id="ts-debug-result" class="toggl-sync__debug-output"></div>
                    </div>
                </div>
            </div>
    ...
    private async startTogglTimer(input: {
        ...
        this.startCloudPromise = this.finishStartTogglTimerCloud({input, start, database, localRowId, prevTimer});

        return true;
    }

    // 后台创建云端计时器。返回创建成功的 TimeEntry；失败或 API 禁用返回 null。
    // 若执行期间用户点了“停止”（this.stopDuringStart=true），则只返回条目、不刷新成“运行中”，
    // 交由 stopCurrentTimer 负责停止云端并写本地停止信息，避免留下孤儿计时器。
    private finishStartTogglTimerCloud(ctx: {
        ...
        if (this.config.apiEnabled === false) return Promise.resolve(null);
        return (async () => {
            ...
            const createBody: any = {
                ...
                description: ctx.input.description || "无描述",
                start: ctx.start.toISOString(),
                duration: -1,
                created_with: "siyuan-toggl-sync",
            };
            if (ctx.input.projectId !== undefined) {
                createBody.project_id = ctx.input.projectId;
            }
            if (ctx.input.tags.length > 0) {
                createBody.tags = ctx.input.tags;
            ...
            // 用户在创建云端计时器的过程中点了停止：不要把状态刷新成“运行中”，
            // 把真实条目交回 stopCurrentTimer，由它停止云端并写本地停止信息。
            if (this.stopDuringStart) {
                return entry;
            }

            // 正常：写入 togglId + 状态“正常”，并刷新 currentTimer
            if (ctx.database && ctx.localRowId) {
                // 防止快速连续“开始”时旧的后台创建结果覆盖新的 currentTimer
                const stillCurrent = this.config.currentTimer
                    && this.config.currentTimer.localRowId === ctx.localRowId;
                if (stillCurrent) {
                    await this.writeTogglId(ctx.database, ctx.localRowId, entry.id);
                    await this.writeSyncStatus(ctx.database, ctx.localRowId, "正常");
                    await this.updateCurrentTimerFromEntry(entry, ctx.input.projectId, ctx.input.tags);
                } else {
                    // 当前计时器已被新的开始替代或清空：只回填 togglId，不再刷新 currentTimer
                    await this.writeTogglId(ctx.database, ctx.localRowId, entry.id);
                }
            }
            this.startCloudPromise = null;
            return entry;
        })().catch((e) => {
    ...
    private async stopCurrentTimer() {
        ...
        let realId = prevTimer.id;
        let workspaceIdForStop = prevTimer.workspaceId || 0;
        const apiEnabled = this.config.apiEnabled !== false;

        // ① 若云端计时器仍在创建中（currentTimer.id 还是 0），先等它创建完成，
        //    拿到真实的 Toggl id 才能去停止，否则会留下一个永远运行的孤儿计时器。
        if (prevTimer.id === 0 && this.startCloudPromise) {
            this.stopDuringStart = true;
            const entry = await this.startCloudPromise;
            this.startCloudPromise = null;
            this.stopDuringStart = false;
            if (entry) {
                realId = entry.id;
                workspaceIdForStop = entry.workspace_id;
                // 把真实 togglId 写回本地行，保证能关联并正确停止
                if (prevTimer.localRowId && prevTimer.databaseAvId) {
                    const database = await this.getTargetDatabase();
                    if (database) {
                        await this.writeTogglId(database, prevTimer.localRowId, entry.id);
                    }
                }
            }
        } else if (this.startCloudPromise) {
            // 云端已创建完成（currentTimer.id 已是真实值），清空挂起的 promise
            this.startCloudPromise = null;
        }
        ...
        const newStatus = realId !== 0 ? "Toggl 待更新" : "本地待上传";
        ...
        if (realId !== 0 && apiEnabled) {
            const workspaceId = workspaceIdForStop || await this.ensureWorkspaceId();
            if (workspaceId) {
                try {
                    if (!prevTimer.localRowId) {
                        // 没有本地行：直接停止，再把停止后的条目写回本地
                        const stopResult = await togglApi.stopTimeEntry(workspaceId, realId);
                        ...
                        const current = await togglApi.getCurrentTimeEntry();
                        if (current.ok && (current.data as any)?.id === realId) {
                            await togglApi.stopTimeEntry(workspaceId, realId);
                        }
                    }
                } catch {
